...
function rScores(cat){
  ...
  let t='<table class="stbl"><tr><th>Measure</th><th class="r">Score</th><th class="r">Peer D'+cur.d+'</th><th class="r">National</th><th class="r">Status</th></tr>';
  for(const s of sc){
    const cl=s[2]===1?'s-g':s[2]===-1?'s-r':'s-n';
    const dt=s[2]===1?'dot-g':s[2]===-1?'dot-r':'dot-y';
    const tx=s[2]===1?'Better':s[2]===-1?'Worse':'No Diff';
    const pv=pm[s[0]]!=null?pm[s[0]]:'—';
    t+=`<tr><td>${SN[s[0]]}</td><td class="r ${cl}">${s[1]}</td><td class="r" style="color:var(--teal)">${pv}</td><td class="r" style="color:var(--text3)">${s.length>3?s[3]:'—'}</td><td class="r"><span class="dot ${dt}"></span><span class="${cl}">${tx}</span></td></tr>`;
  }
  w.innerHTML=t+'</table>';
}
function rComm(h){
