...
function rComm(h){
  ...
  el.innerHTML=order.filter(k=>h.ch[k]!=null).map(k=>{
    ...
    return`<div class="br"><div class="br-l">${k}</div><div class="br-t"><div class="br-f" style="width:${pct}%;background:${col};"></div><div class="br-m" style="left:${npct}%;"></div><div class="br-v" style="color:${col};">${v}%</div></div></div>`;
  }).join('');
}
